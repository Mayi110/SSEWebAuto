...
class BasePage(object):
    ...
    def getElements(self,locateType,locatorExpression):
        '''
        获取多个元素对象
        :param locateType:定位表达式
        :param locatorExpression;元素表达式
        ...
        try:
            # global driver
            elements = WebDriverWait(self.driver,30).until(lambda x:x.find_elements(by=locateType, value=locatorExpression))
            return elements
        except Exception as e:
            raise e
    ...
    def clear(self, locateType, locatorExpression):
        ...
        try:
            ...
            # element.clear()
            self.getElement(locateType, locatorExpression).clear()
        except Exception as e:
            raise e
    ...
    def input_string(self, locateType, locatorExpression, inputContent):
        ...
        try:
            ...
            self.getElement(locateType, locatorExpression).send_keys(inputContent)
        except Exception as e:
            raise e
    ...
    def click(self, locateType, locatorExpression, *arg):
        ...
        try:
            # element = WebDriverWait(driver,30).until(lambda x:x.find_element(by=locateType,value=locatorExpression))
            # element.click()
            self.getElement(locateType, locatorExpression).click()
        except Exception as e:
            raise e
    ...
    def selectByValue(self, locateType, locatorExpression, value, *arg):
        ...
        try:
            # element = WebDriverWait(driver,30).until(lambda x:x.find_element(by=locateType,value=locatorExpression))
            elementObj = self.getElement(locateType, locatorExpression)
